...
void FlatLargeHornifyFunction::runOnFunction(Function &F) {
  ...
  for (const CutPoint &cp : cpg) {
    for (const CpEdge *edge :
      ...
      s.write(pc, mkTerm<expr::mpz_class>((unsigned long)cpgOrder[&cp.bb()],
                                          m_efac));
      args.push_back(s.read(pc));
      for (const Expr &v : glive)
        args.push_back(s.read(v));
      ...
      s.write(pc,
              mkTerm<expr::mpz_class>((unsigned long)cpgOrder[&dst], m_efac));
      args.push_back(s.read(pc));
      for (const Expr &v : glive)
        args.push_back(s.read(v));
      allVars.insert(++args.begin(), args.end());
  ...
  for (const CutPoint &cp : cpg) {
    ...
    s.write(pc,
            mkTerm<expr::mpz_class>((unsigned long)cpgOrder[&cp.bb()], m_efac));
    args.push_back(s.read(pc));
    for (const Expr &v : glive)
      args.push_back(s.read(v));
    allVars.insert(++args.begin(), args.end());
