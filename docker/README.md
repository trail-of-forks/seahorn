...
```shell
$ docker build  -t seahorn/buildpack-deps-seahorn:jammy-llvm20 -f docker/buildpack-deps-seahorn.Dockerfile .
```
...
```shell
$ docker build  -t seahorn/seahorn-builder:jammy-llvm20 -f docker/seahorn-builder.Dockerfile .
$ docker run -v $(pwd):/host --rm -it seahorn/seahorn-builder:jammy-llvm20 /bin/sh -c "cp build/*.tar.gz /host/"
```
...
```shell
$ docker build  -t seahorn/seahorn-llvm20:latest -f docker/seahorn.Dockerfile .
$ docker run --rm -it seahorn/seahorn-llvm20:latest
```
